...
contract RedirectAll is SuperAppBase {
        ...
        address receiver)
    {
        ...
        boundless = Boundless(_boundless);
    ...
    function afterAgreementCreated(
        ...
        returns (bytes memory newCtx)
    {
        uint256 id = abi.decode(host.decodeCtx(_ctx).userData, (uint256));
        if (boundless.balanceOf(address(seller), tokenId)) {
            seller.buyToken(_host.decodeCtx(_ctx).msgSender, address(boundless), id);
            return _updateOutflow(_ctx);
        } else {
            boundless.buyToken(_host.decodeCtx(_ctx).msgSender, id);
            return _updateOutflow(_ctx);
        }
    }
